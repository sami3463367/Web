...
import { all, get, run, tx, getSettings, setSetting } from '../core/db.js';
...
router.get('/api/admin/stats', (req, res) => {
  ...
  const totals = get(`
    SELECT
      ...
      COALESCE(SUM(CASE WHEN o.status != 'cancelled' THEN 1 ELSE 0 END),0) AS orders_all
    FROM orders o`);
  ...
  const statusSplit = all(`SELECT status, COUNT(*) AS count FROM orders GROUP BY status ORDER BY count DESC`);
  ...
  const inventory = get(`SELECT COALESCE(SUM(stock),0) AS units, COUNT(*) AS skus,
                         COUNT(CASE WHEN stock = 0 THEN 1 END) AS out_of_stock
                         FROM products WHERE status='active'`);

  const recentOrders = all(`SELECT * FROM orders ORDER BY created_at DESC, id DESC LIMIT 6`);
...
router.get('/api/admin/orders', (req, res) => {
  ...
  const per = 15;
  const total = get(`SELECT COUNT(*) AS c FROM orders o WHERE ${where.join(' AND ')}`, ...params).c;
  const rows = all(
...
router.get('/api/admin/orders/:id', (req, res) => {
  const order = get('SELECT * FROM orders WHERE id = ?', asInt(req.params.id, 0));
...
router.patch('/api/admin/orders/:id', async (req, res) => {
  ...
  tx(() => {
    ...
    const items = all('SELECT product_id, qty FROM order_items WHERE order_id = ?', id);
    const wasCancelled = order.status === 'cancelled';

    if (status === 'cancelled' && !wasCancelled) {
      for (const it of items) {
        if (it.product_id) run('UPDATE products SET stock = stock + ?, sold = MAX(0, sold - ?) WHERE id = ?', it.qty, it.qty, it.product_id);
      }
    }
    if (wasCancelled && status !== 'cancelled') {
      for (const it of items) {
        if (!it.product_id) continue;
        const p = get('SELECT stock, name FROM products WHERE id = ?', it.product_id);
        if (!p || p.stock < it.qty) throw new HttpError(409, `Not enough stock to reopen this order (${p ? p.name : 'item'})`);
        run('UPDATE products SET stock = stock - ?, sold = sold + ? WHERE id = ?', it.qty, it.qty, it.product_id);
      }
    }
  ...
  const order = get('SELECT * FROM orders WHERE id = ?', id);
  const items = all('SELECT * FROM order_items WHERE order_id = ? ORDER BY id', id);
  res.json(200, { order, items });
});
...
function validateProductPayload(body, partial = false) {
  ...
  if (body.stock !== undefined) {
    out.stock = Math.max(0, asInt(body.stock, 0));
  }
  ...
  if (body.category_id !== undefined) {
    const cat = get('SELECT id FROM categories WHERE id = ?', asInt(body.category_id, 0));
...
router.post('/api/admin/products', async (req, res) => {
  ...
  if (get('SELECT id FROM products WHERE slug = ?', slug)) slug = `${slug}-${Date.now().toString(36).slice(-4)}`;
  const categoryId = data.category_id || get('SELECT id FROM categories ORDER BY sort LIMIT 1').id;
  const result = run(
  ...
  res.json(201, parseProduct(get(`SELECT ${PRODUCT_COLUMNS} ${PRODUCT_JOIN} WHERE p.id = ?`, Number(result.lastInsertRowid))));
});
...
router.patch('/api/admin/products/:id', async (req, res) => {
  ...
  run(`UPDATE products SET ${sets}, updated_at = datetime('now') WHERE id = ?`, ...fields.map((f) => data[f]), id);
  res.json(200, parseProduct(get(`SELECT ${PRODUCT_COLUMNS} ${PRODUCT_JOIN} WHERE p.id = ?`, id)));
});
...
router.post('/api/admin/products/:id/stock', async (req, res) => {
  const id = asInt(req.params.id, 0);
  const body = await readJson(req);
  const product = get('SELECT id, stock, name FROM products WHERE id = ?', id);
  ...
  run(`UPDATE products SET stock = ?, updated_at = datetime('now') WHERE id = ?`, next, id);
...
router.delete('/api/admin/products/:id', (req, res) => {
  ...
  run(`UPDATE products SET status = 'archived', updated_at = datetime('now') WHERE id = ?`, id);
  res.json(200, { ok: true });
});

router.post('/api/admin/products/:id/restore', (req, res) => {
  const id = asInt(req.params.id, 0);
  run(`UPDATE products SET status = 'active', updated_at = datetime('now') WHERE id = ? AND status = 'archived'`, id);
  res.json(200, { ok: true });
});

router.get('/api/admin/categories', (req, res) => {
  res.json(200, all('SELECT * FROM categories ORDER BY sort, name'));
});
