...
router.get('/api/me', (req, res) => {
  res.json(200, { user: publicUser(currentUser(req)) });
});
...
router.post('/api/auth/register', async (req, res) => {
  ...
  const user = get('SELECT id,name,phone,email,address,role,created_at FROM users WHERE id=?', Number(result.lastInsertRowid));
...
router.post('/api/auth/login', async (req, res) => {
  ...
  const user = identifier.includes('@')
    ? get('SELECT * FROM users WHERE email = ?', identifier)
    : get('SELECT * FROM users WHERE phone = ?', identifier.replace(/^\+?880/, '0').replace(/^880/, '0') || identifier);
...
router.patch('/api/me', async (req, res) => {
  ...
  if (email && email !== user.email && get('SELECT id FROM users WHERE email=? AND id!=?', email, user.id)) {
    throw new HttpError(409, 'That email is already in use', { field: 'email' });
  }
  run('UPDATE users SET name=?, address=?, email=? WHERE id=?', name, address, email, user.id);
  res.json(200, { user: publicUser(get('SELECT id,name,phone,email,address,role,created_at FROM users WHERE id=?', user.id)) });
});
...
router.post('/api/auth/password', async (req, res) => {
  ...
  run('UPDATE users SET password_hash=? WHERE id=?', hashPassword(requirePassword(body.next)), user.id);
  res.json(200, { ok: true });
});
